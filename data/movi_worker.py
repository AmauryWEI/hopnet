...
SPAWN_REGION = [(-5.0, -5.0, 1.0), (5.0, 5.0, 5.0)]
...
parser.add_argument(
    "--objects_set", choices=["clevr", "kubasic", "spheres"], default="clevr"
)
parser.add_argument(
...
floor_material = kb.PrincipledBSDFMaterial(roughness=1.0, specular=0.0)
scene += kubasic.create(
    "dome",
    name="floor",
...
for i in range(num_objects):
    if FLAGS.objects_set == "spheres":
        ...
        raise RuntimeError(f"Unknown objects set: {FLAGS.objects_set}")
...
# Save to image files
kb.write_image_dict(data_stack, output_dir, max_write_threads=cpu_count())
